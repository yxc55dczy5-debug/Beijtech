...
try {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($action === 'product_save') {
            ...
            if ($values[0] === '') {
                throw new RuntimeException('Productnaam is verplicht.');
            }
        ...
        if ($action === 'product_toggle') {
            ...
            db_run(
                'UPDATE products SET active = IF(active = 1, 0, 1), updated_at = NOW() WHERE id = ?',
                [$id]
            );
        ...
        if ($action === 'project_save') {
            ...
            if ($id > 0) {
                ...
                db_run(
                    'INSERT INTO projects (naam, klant, locatie, date_from, date_to, status, opmerkingen) VALUES (?, ?, ?, ?, ?, ?, ?)',
                    $values
                );
        ...
        if ($action === 'user_save') {
            ...
            if ($username === '' || $email === '') {
                throw new RuntimeException('Gebruikersnaam en e-mail zijn verplicht.');
            }
            ...
            if ($id > 0) {
                if ($password !== '') {
                    db_run(
                        ...
                        [$username, $email, $role, password_hash($password, PASSWORD_BCRYPT), $id]
                    );
                } else {
                    db_run(
                ...
                if ($password === '') {
                    throw new RuntimeException('Wachtwoord is verplicht voor nieuwe gebruikers.');
                }

                db_run(
                    'INSERT INTO users (username, email, password_hash, role) VALUES (?, ?, ?, ?)',
                    [$username, $email, password_hash($password, PASSWORD_BCRYPT), $role]
                );
...
$editProduct = isset($_GET['edit_product'])
    ? db_get('SELECT * FROM products WHERE id = ?', [(int)$_GET['edit_product']])
    : null;

$editProject = isset($_GET['edit_project'])
    ? db_get('SELECT * FROM projects WHERE id = ?', [(int)$_GET['edit_project']])
    : null;

$editUser = isset($_GET['edit_user'])
    ? db_get('SELECT id, username, email, role FROM users WHERE id = ?', [(int)$_GET['edit_user']])
    : null;
    ...
    <div class="min-h-screen grid grid-cols-1 lg:grid-cols-[240px_1fr]">
        <aside class="border-r border-[#252525] bg-[#0f0f0f] p-4">
            <div class="text-xl font-black mb-6">
                <span>BEIJ</span><span class="bg-[#C9A028] text-black px-1 ml-1">TECH</span>
            </div>

            <nav class="space-y-2">
                <?php foreach (['dashboard' => 'Dashboard', 'projects' => 'Projecten', 'products' => 'Producten', 'quotes' => 'Offertes', 'users' => 'Gebruikers'] as $key => $label): ?>
                    <a
                        class="btn w-full justify-start <?= $tab === $key ? 'nav-active' : 'btn-dark' ?>"
                        href="<?= e(admin_url($key)) ?>"
                    >
                        <?= e($label) ?>
                    </a>
                <?php endforeach; ?>
        ...
        <main class="p-4 lg:p-8">
            <?php if ($flash): ?>
                <div class="mb-5 rounded-lg px-4 py-3 text-sm font-bold <?= $flash['type'] === 'err' ? 'bg-red-950 text-red-200' : 'bg-green-950 text-green-200' ?>">
                    <?= e($flash['message']) ?>
                </div>
            <?php endif; ?>

            <?php foreach ($loadErrors as $loadError): ?>
                <div class="mb-3 rounded-lg px-4 py-3 text-sm font-bold bg-red-950 text-red-200">
                    <?= e($loadError) ?>
                </div>
            <?php endforeach; ?>
            ...
            <?php if ($tab === 'dashboard'): ?>
                ...
                <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    <div class="panel p-5">
                        <p class="text-3xl font-black"><?= $stats['projects'] ?></p>
                        <p class="text-gray-500 text-sm">Projecten</p>
                    </div>

                    <div class="panel p-5">
                        <p class="text-3xl font-black"><?= $stats['products'] ?></p>
                        <p class="text-gray-500 text-sm">Producten</p>
                    </div>

                    <div class="panel p-5">
                        <p class="text-3xl font-black"><?= $stats['quotes'] ?></p>
                        <p class="text-gray-500 text-sm">Offertes</p>
                    </div>
                ...
                <section class="panel p-5 mt-6">
                    ...
                    <?php if (empty($quotes)): ?>
                        <p class="text-gray-500 text-sm">Nog geen offerte-aanvragen.</p>
                    <?php endif; ?>
                    ...
                    <?php foreach (array_slice($quotes, 0, 5) as $q): ?>
                        <div class="border-t border-[#242424] py-3 flex justify-between gap-4">
                            <span>
                                <?= e($q['naam']) ?>
                                <span class="text-gray-500"><?= e($q['email']) ?></span>
                            </span>
            ...
            <?php if ($tab === 'products'): ?>
                    ...
                    <form method="post" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-3 gap-3">
                        ...
                        <select class="field" name="category">
                            <?php foreach ($catLabels as $k => $v): ?>
                                <option value="<?= e($k) ?>" <?= (($editProduct['category'] ?? '') === $k) ? 'selected' : '' ?>>
                                    <?= e($v) ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                        ...
                        <input
                            ...
                            value="<?= e($editProduct['stock_quantity'] ?? 0) ?>"
                        >
                        ...
                        <input
                            ...
                            value="<?= e($editProduct['sort_order'] ?? 0) ?>"
                        >

                        <label class="field">
                            <input type="checkbox" name="popular" <?= !empty($editProduct['popular']) ? 'checked' : '' ?>>
                            Populair
                        </label>

                        <label class="field">
                            <input type="checkbox" name="active" <?= !isset($editProduct) || !empty($editProduct['active']) ? 'checked' : '' ?>>
                            Actief
                        </label>
                        ...
                        <?php if ($editProduct): ?>
                            <a class="btn btn-dark" href="<?= e(admin_url('products')) ?>">Annuleren</a>
                        <?php endif; ?>
                    </form>
                </section>
                ...
                <div class="panel overflow-hidden">
                    <?php if (empty($products)): ?>
                        <div class="p-4 text-gray-500 text-sm">Nog geen producten.</div>
                    <?php endif; ?>
                        ...
                        <div class="p-4 border-b border-[#242424] flex flex-col lg:flex-row lg:items-center gap-3 justify-between">
                            <div>
                                ...
                                <p class="text-sm text-gray-500">
                                    <?= e($catLabels[$p['category']] ?? $p['category']) ?>
                                    · voorraad <?= e($p['stock_quantity']) ?>
                                    · volgorde <?= e($p['sort_order']) ?>
                                </p>
                            </div>
                            ...
                            <div class="flex gap-2 flex-wrap">
                                ...
                                <a class="btn btn-dark" href="dashboard.php?tab=products&edit_product=<?= (int)$p['id'] ?>">
                                    Bewerken
                                </a>
                                ...
                                <form method="post" onsubmit="return confirm('Weet je zeker dat je dit product wilt verwijderen?');">
                                    ...
                                    <input type="hidden" name="id" value="<?= (int)$p['id'] ?>">
                                    <button class="btn btn-red" type="submit">Verwijderen</button>
                                </form>
                            </div>
                        </div>
                    <?php endforeach; ?>
            ...
            <?php if ($tab === 'quotes'): ?>
                ...
                <div class="space-y-4">
                    <?php if (empty($quotes)): ?>
                        <section class="panel p-5 text-gray-500 text-sm">
                            Nog geen offertes.
                        </section>
                    <?php endif; ?>
                    ...
                    <?php foreach ($quotes as $q): ?>
                        <?php
                        try {
                            $items = db_all('SELECT * FROM quote_items WHERE quote_id = ?', [$q['id']]);
                        } catch (Throwable $e) {
                            $items = [];
                        }
                        ?>
                            ...
                            <div class="flex flex-col lg:flex-row justify-between gap-4">
                                <div>
                                    ...
                                    <?php foreach ($items as $item): ?>
                                        <p class="text-xs text-gray-500 mt-1">
                                            <?= e($item['quantity']) ?>x <?= e($item['product_name']) ?>
                                        </p>
                                    <?php endforeach; ?>
                                </div>
                                ...
                                <form method="post" class="space-y-2 min-w-[260px]">
                                    ...
                                    <select class="field" name="status">
                                        <?php foreach ($quoteStatuses as $k => $v): ?>
                                            <option value="<?= e($k) ?>" <?= $q['status'] === $k ? 'selected' : '' ?>>
                                                <?= e($v) ?>
                                            </option>
                                        <?php endforeach; ?>
                                    </select>
                                ...
                                <form method="post" onsubmit="return confirm('Weet je zeker dat je deze offerte wilt verwijderen?');">
                                    <input type="hidden" name="csrf" value="<?= e($csrf) ?>">
            ...
            <?php if ($tab === 'projects'): ?>
                ...
                <section class="panel p-5 mb-6">
                    <h2 class="font-black mb-4">
                        <?= $editProject ? 'Project bewerken' : 'Project toevoegen' ?>
                    </h2>
                    ...
                    <form method="post" class="grid grid-cols-1 lg:grid-cols-4 gap-3">
                        ...
                        <input
                            ...
                            name="locatie"
                            placeholder="Locatie"
                            value="<?= e($editProject['locatie'] ?? '') ?>"
                        >

                        <select class="field" name="status">
                            <?php foreach ($projectStatuses as $k => $v): ?>
                                <option value="<?= e($k) ?>" <?= (($editProject['status'] ?? '') === $k) ? 'selected' : '' ?>>
                                    <?= e($v) ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                        ...
                        <?php if ($editProject): ?>
                            <a class="btn btn-dark" href="<?= e(admin_url('projects')) ?>">Annuleren</a>
                        <?php endif; ?>
                    </form>
                </section>
                ...
                <div class="panel overflow-hidden">
                    <?php if (empty($projects)): ?>
                        <div class="p-4 text-gray-500 text-sm">Nog geen projecten.</div>
                    <?php endif; ?>

                    <?php foreach ($projects as $p): ?>
                        <div class="p-4 border-b border-[#242424] flex flex-col lg:flex-row lg:items-center gap-3 justify-between">
                            <div>
                                <p class="font-bold"><?= e($p['naam']) ?></p>
                                <p class="text-sm text-gray-500">
                                    <?= e($p['date_from']) ?> t/m <?= e($p['date_to']) ?>
                                    · <?= e($p['klant'] ?? '') ?>
                                    · <?= e($p['locatie'] ?? '') ?>
                                </p>
                            </div>
                            ...
                            <div class="flex gap-2 flex-wrap">
                                <span class="badge">
                                    <?= e($projectStatuses[$p['status']] ?? $p['status']) ?>
                                </span>

                                <a class="btn btn-dark" href="dashboard.php?tab=projects&edit_project=<?= (int)$p['id'] ?>">
                                    Bewerken
                                </a>
            ...
            <?php if ($tab === 'users'): ?>
                ...
                <section class="panel p-5 mb-6">
                    <h2 class="font-black mb-4">
                        <?= $editUser ? 'Gebruiker bewerken' : 'Gebruiker toevoegen' ?>
                    </h2>
                    ...
                    <form method="post" class="grid grid-cols-1 lg:grid-cols-4 gap-3">
                        ...
                        <input
                            ...
                            type="password"
                            name="password"
                            placeholder="<?= $editUser ? 'Nieuw wachtwoord optioneel' : 'Wachtwoord' ?>"
                        >

                        <button class="btn btn-gold" type="submit">Opslaan</button>

                        <?php if ($editUser): ?>
                            <a class="btn btn-dark" href="<?= e(admin_url('users')) ?>">Annuleren</a>
                        <?php endif; ?>
                    </form>
                </section>
                ...
                <div class="panel overflow-hidden">
                    <?php if (empty($users)): ?>
                        <div class="p-4 text-gray-500 text-sm">Nog geen gebruikers.</div>
                    <?php endif; ?>
                        ...
                        <div class="p-4 border-b border-[#242424] flex flex-col lg:flex-row lg:items-center gap-3 justify-between">
                            <div>
                                ...
                                <p class="text-xs text-gray-600 mt-1">
                                    Laatste login:
                                    <?= !empty($u['last_login']) ? e($u['last_login']) : 'Nog nooit' ?>
                                </p>
                            </div>

                            <div class="flex gap-2 flex-wrap">
                                <a class="btn btn-dark" href="dashboard.php?tab=users&edit_user=<?= (int)$u['id'] ?>">
                                    Bewerken
                                </a>
